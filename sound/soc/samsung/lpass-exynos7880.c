/*
 * Audio SubSystem driver for Samsung Exynos7880
 *
...
#define EXYNOS7880_DISPAUD_CFG			0x1000
...
void lpass_update_lpclock_impl(struct device *dev, u32 ctrlid, bool active)
{
	...
	if (g_init_sicd_aud_index == 0) {
		g_sicd_aud_index = exynos_get_idle_ip_index("148f0000.lpass");
		g_init_sicd_aud_index = 1;
	}
	if (g_sicd_aud_index < 0) {
 ...
 */
int lpass_set_clk_hierarchy(struct device *dev)
{
...
static const struct of_device_id exynos_lpass_match[] = {
	{
		.compatible	= "samsung,exynos7880-lpass",
		.data		= &lpass_ver_data[LPASS_VER_7880],
	}, {},
};
