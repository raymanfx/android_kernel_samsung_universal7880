...
#define COD3026X_WATER_DET_THRESHOLD_MIN_FIRST 150
#define COD3026X_WATER_DET_THRESHOLD_MIN 110
#define COD3026X_JACK_IN_THRESHOLD 50
#define COD3026X_WATER_DET_POLLING_TIME    1000

#define COD3026X_RESETB_DSML_COUNT 2
...
static int cod3026_power_on_mic1(struct snd_soc_codec *codec)
{
	...
	for (i = 0; i < COD3026X_RESETB_DSML_COUNT; i++) {
		snd_soc_update_bits(codec, COD3026X_11_PD_AD1,
...
static int cod3026x_jack_in_chk_more(struct cod3026x_priv *cod3026x)
{
	...
	for ( i=0; i<JACK_IN_CHK_MORE_NO; i++) {
		...
		if( gdet_adc == 0 || gdet_adc > COD3026X_WATER_DET_THRESHOLD_MIN )
			jack_in_det = 0;
	}
...
static void cod3026x_water_det_work(struct work_struct *work)
{
	...
	if (gdet_adc != 0 && gdet_adc >= COD3026X_WATER_DET_THRESHOLD_MIN_FIRST) {
		if (gdet_adc < COD3026X_WATER_DET_THRESHOLD_MAX) {
			dev_dbg(cod3026x->dev, "%s water is detected.\n" , __func__);
			snd_soc_write(codec, 0x84, 0x0d);
			waterdet->water_det = COD3026X_DET_WATER;
...
static irqreturn_t cod3026x_threaded_isr(int irq, void *data)
{
	...
	if (det_status_change) {
		if (cod3026x->use_det_adc_mode) {
			...
			/* mic detection delay */
			if (jd->jack_det) {
				cancel_delayed_work(&cod3026x->jack_det_adc_work);
				queue_delayed_work(cod3026x->jack_det_adc_wq,
						&cod3026x->jack_det_adc_work, msecs_to_jiffies(cod3026x->mic_det_delay));
			} else
				queue_delayed_work(cod3026x->jack_det_adc_wq,
			...
			/* mic detection delay */
			if (jd->jack_det) {
				cancel_delayed_work(&cod3026x->jack_det_work);
...
static void cod3026x_post_fw_update_failure(void *context)
{
	...
	snd_soc_write(codec, COD3026X_75_AD_MIC3N_TRIM1, 0x13);
