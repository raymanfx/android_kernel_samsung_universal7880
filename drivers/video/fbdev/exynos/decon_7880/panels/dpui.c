...
static struct dpui_info dpui = {
	...
	.field = {
		/* common hw parameter */
		DPUI_FIELD_INIT(DPUI_LOG_LEVEL_INFO, DPUI_TYPE_MDNIE, DPUI_KEY_WCRD),
		DPUI_FIELD_INIT(DPUI_LOG_LEVEL_INFO, DPUI_TYPE_MDNIE, DPUI_KEY_WOFS),

		/* common hw parameter - for debug */
		DPUI_FIELD_INIT(DPUI_LOG_LEVEL_DEBUG, DPUI_TYPE_MDNIE, DPUI_KEY_WRGB),
...
/**
 * dpui_logging_notify - notify clients of dpui
 * @val: dpui log type
 * @v : data
 *
 */
...
int get_dpui_field(enum dpui_key key, char *buf)
{
	...
	return scnprintf(buf, MAX_DPUI_KEY_LEN + MAX_DPUI_VAL_LEN,
...
int get_dpui_log(char *buf, enum dpui_log_level level)
{
	...
	for (i = DPUI_KEY_NONE + 1; i < MAX_DPUI_KEY; i++) {
		...
		if (len)
			len += scnprintf(buf + len, 3, ", ");
		len += scnprintf(buf + len, MAX_DPUI_KEY_LEN + MAX_DPUI_VAL_LEN,
				"%s", tbuf);
	}
