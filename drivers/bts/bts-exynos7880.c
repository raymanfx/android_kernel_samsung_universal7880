...
#include "cal_bts7880.h"
#include "regs-bts.h"

#define WQHD_BW			(2560 * 1440 * 4 * 60)
...
#define VPP_IN_IC_MAX	0x10

#define AW_IC_TARGET_M0	0x1
#define AW_AC_TARGET_S1	0x1
#define AW_AC_TARGET_S0	0x20

#define AW_IC_M0	0x6
#define AR_IC_M0	0x6
#define AW_AC_S0	0x6
#define AR_AC_S0	0x6
#define AW_AC_S1	0x30
#define AR_AC_S1	0x30

#define D0_AR_AC_CP_S2	0x20
#define D0_AR_AC_NRT_S1	0xC
#define D0_AR_AC_RT_S0	0x20
#define D0_AR_IC_M0	0xC

#define D0_AW_AC_CP_S2	0x20
#define D0_AW_AC_NRT_S1	0x6
#define D0_AW_AC_RT_S0	0x20
#define D0_AW_IC_M0	0x6

#define D1_AR_AC_CP_S2	0x20
#define D1_AR_AC_NRT_S1	0xC
#define D1_AR_AC_RT_S0	0x20
#define D1_AR_IC_M0	0xC

#define D1_AW_AC_CP_S2	0x20
#define D1_AW_AC_NRT_S1	0x6
#define D1_AW_AC_RT_S0	0x20
#define D1_AW_IC_M0	0x6
...
enum bts_id {
	...
	BTS_MFCMSCL0 = (1 << BTS_IDX_MFCMSCL0),
	BTS_MFCMSCL1 = (1 << BTS_IDX_MFCMSCL1),
...
static struct pm_qos_request exynos7_winlayer_mif_bts_qos;
static struct srcu_notifier_head exynos_media_notifier;
static struct clk_info clk_table[] = {
{"gate_dispaud_bts_alias", NULL, BTS_IDX_DISPAUD},
{"gate_isp_bts_alias", NULL, BTS_IDX_ISP0},
{"gate_mfcmscl_bts_alias", NULL, BTS_IDX_MFCMSCL0},
};

static DEFINE_MUTEX(media_mutex);
static unsigned int decon_bw, cam_bw, total_bw;
static void __iomem *base_drex0, *base_drex1;
static void __iomem *base_bts_pmu_alive;
static void __iomem *base_sysreg_ccore;
static void __iomem *base_vpp_mo;
static void __iomem *base_isp_xiu;
static void __iomem *base_mfcscl_sysreg;
static unsigned int mif_freq;
static unsigned int int_freq;

static struct bts_info exynos7_bts[] = {
	[BTS_IDX_DISPAUD] = {
		.id = BTS_DISPAUD,
		.name = "disp",
		.pa_base = EXYNOS7880_PA_SYSREG_DISPAUD,
		.pd_name = "pd-dispaud",
	...
	[BTS_IDX_ISP0] = {
		.id = BTS_ISP0,
		.name = "isp0",
		.pa_base = EXYNOS7880_PA_SYSREG_ISP,
		.pd_name = "pd-isp",
		.table[BS_DEFAULT].fn = BF_SETQOS,
		.table[BS_DEFAULT].priority[0] = 0x6,
		.table[BS_DEFAULT].priority[1] = 0x6,
		.table[BS_CAM_BNS].fn = BF_SETQOS,
		.table[BS_CAM_BNS].priority[0] = 0x6,
		.table[BS_CAM_BNS].priority[1] = 0x6,
		.table[BS_DISABLE].fn = BF_SETQOS,
		.table[BS_DISABLE].priority[0] = 0x4,
	...
	[BTS_IDX_MFCMSCL0] = {
		.id = BTS_MFCMSCL0,
		.name = "mfc",
		.pa_base = EXYNOS7880_PA_SYSREG_MFCMSCL,
		.pd_name = "pd-mfcmscl",
		.table[BS_DEFAULT].fn = BF_SETQOS,
	...
	[BTS_IDX_FSYS0] = {
		.id = BTS_FSYS0,
		.name = "fsys0",
		.pa_base = EXYNOS7880_PA_SYSREG_FSYS,
		.pd_name = "pd-fsys",
	...
	[BTS_IDX_MODAPIF_CP] = {
		.id = BTS_MODAPIF_CP,
		.name = "modapif_cp",
		.pa_base = EXYNOS7880_PA_PMU_ALIVE,
		.pd_name = "pd-modapif",
		.table[BS_DEFAULT].fn = BF_SETQOS,
	...
	[BTS_IDX_CPU] = {
		.id = BTS_CPU,
		.name = "cpu",
		.pa_base = EXYNOS7880_PA_SYSREG_CCORE,
		.pd_name = "pd-cpu",
		.table[BS_DEFAULT].fn = BF_SETQOS,
		.table[BS_DEFAULT].priority[0] = 0x6,
		.table[BS_DISABLE].fn = BF_SETQOS,
		.table[BS_DISABLE].priority[0] = 0x6,
		.cur_scen = BS_DISABLE,
		.on = false,
		.enable = true,
	},
...
static void bts_vpp_mo_init(void __iomem *base)
{
	unsigned int val_r;
	BTS_DBG("[BTS][%s] bts vpp mo init\n", __func__);

	val_r = __raw_readl(base + IDMA_G0_IN_CON);
	val_r &= ~((0x1f << 19));
	val_r |= (VPP_IN_IC_MAX << 19);
	__raw_writel(val_r, base + IDMA_G0_IN_CON);

	val_r = __raw_readl(base + IDMA_G1_IN_CON);
	val_r &= ~((0x1f << 19));
	val_r |= (VPP_IN_IC_MAX << 19);
	__raw_writel(val_r, base + IDMA_G1_IN_CON);

	val_r = __raw_readl(base + IDMA_G2_IN_CON);
	val_r &= ~((0x1f << 19));
	val_r |= (VPP_IN_IC_MAX << 19);
	__raw_writel(val_r, base + IDMA_G2_IN_CON);

	val_r = __raw_readl(base + IDMA_VG0_IN_CON);
	val_r &= ~((0x1f << 19));
	val_r |= (VPP_IN_IC_MAX << 19);
	__raw_writel(val_r, base + IDMA_VG0_IN_CON);
}

static void bts_isp_xiu_init(void __iomem *base)
{
	unsigned int val_r;
	BTS_DBG("[BTS][%s] bts isp xiu init\n", __func__);

	val_r = __raw_readl(base + XIU_IS1_TARGET);
	val_r &= ~((0x7f << 13) | (0x7f << 6) | (0x3f)) ;
	val_r |= ((AW_IC_TARGET_M0 << 13) | (AW_AC_TARGET_S1 << 6) | (AW_AC_TARGET_S0));
	__raw_writel(val_r, base + XIU_IS1_TARGET);
}
					...
					unsigned int *priority)
{
	...
	case BTS_ISP1:
		...
		break;
	case BTS_MFCMSCL0:
		bts_setqos_sysreg(BTS_SYSREG_MFCMSCL0, va_base, priority);
		break;
	case BTS_MFCMSCL1:
		bts_setqos_sysreg(BTS_SYSREG_MFCMSCL1, va_base, priority);
		break;
	case BTS_FSYS0:
	...
	case BTS_FSYS2:
		...
		break;
	case BTS_MODAPIF_CP:
		bts_setqos_sysreg(BTS_SYSREG_MODAPIF_CP, va_base, priority);
		break;
	case BTS_MODAPIF_GNSS:
		bts_setqos_sysreg(BTS_SYSREG_MODAPIF_GNSS, va_base, priority);
		break;
	case BTS_CPU:
		bts_setqos_sysreg(BTS_SYSREG_CCORE_CPU, va_base, priority);
		break;
	case BTS_APL:
		bts_setqos_sysreg(BTS_SYSREG_CCORE_APL, va_base, priority);
		break;
	default:
		break;
	}
	...
	case BF_SETQOS:
		...
		if (bts->id == BTS_ISP0) {
			...
			bts_setotf_sysreg(BTS_SYSREG_ISP_ISP_SCL_SEL, \
					bts->va_base, on);
			bts_isp_xiu_init(base_isp_xiu);
		}
		if (bts->id == BTS_DISPAUD) {
			bts_vpp_mo_init(base_vpp_mo);
		}
		break;
	case BF_SETQOS_BW:
		break;
	...
	for (i = 0; i < ARRAY_SIZE(exynos7_bts); i++) {
		...
		if (exynos7_bts[i].on) {
			...
			case BTS_ISP1:
				...
				break;
			case BTS_MFCMSCL0:
				val_r = __raw_readl(exynos7_bts[i].va_base + MFCMSCL_QOS_CON_MFC);
				break;
			case BTS_MFCMSCL1:
				val_r = __raw_readl(exynos7_bts[i].va_base + MFCMSCL_QOS_CON_MSCL);
				break;
			case BTS_FSYS0:
			...
			case BTS_MODAPIF_GNSS:
				...
				break;
			case BTS_CPU:
				val_r = __raw_readl(exynos7_bts[i].va_base + CCORE_CPU_QOS_CON);
				break;
			case BTS_APL:
				val_r = __raw_readl(exynos7_bts[i].va_base + CCORE_APL_QOS_CON);
				break;
			default:
				break;
			}
...
static void bts_xiu_init(void __iomem *base)
{
	unsigned int val_r;
	BTS_DBG("[BTS][%s] bts xiu init\n", __func__);

	val_r = __raw_readl(base + XIU0_AR_TIDEMARK);
	val_r &= ~((0x3f << 24) | (0x3f << 16) | (0x3f << 8) | (0x3f)) ;
	val_r |= ((D0_AR_AC_CP_S2 << 24) | (D0_AR_AC_NRT_S1 << 16) | (D0_AR_AC_RT_S0 << 8) | (D0_AR_IC_M0)) ;
	__raw_writel(val_r, base + XIU0_AR_TIDEMARK);

	val_r = __raw_readl(base + XIU0_AW_TIDEMARK);
	val_r &= ~((0x3f << 24) | (0x3f << 16) | (0x3f << 8) | (0x3f)) ;
	val_r |= ((D0_AW_AC_CP_S2 << 24) | (D0_AW_AC_NRT_S1 << 16) | (D0_AW_AC_RT_S0 << 8) | (D0_AW_IC_M0)) ;
	__raw_writel(val_r, base + XIU0_AW_TIDEMARK);

	val_r = __raw_readl(base + XIU1_AR_TIDEMARK);
	val_r &= ~((0x3f << 24) | (0x3f << 16) | (0x3f << 8) | (0x3f)) ;
	val_r |= ((D1_AR_AC_CP_S2 << 24) | (D1_AR_AC_NRT_S1 << 16) | (D1_AR_AC_RT_S0 << 8) | (D1_AR_IC_M0)) ;
	__raw_writel(val_r, base + XIU1_AR_TIDEMARK);

	val_r = __raw_readl(base + XIU1_AW_TIDEMARK);
	val_r &= ~((0x3f << 24) | (0x3f << 16) | (0x3f << 8) | (0x3f)) ;
	val_r |= ((D1_AW_AC_CP_S2 << 24) | (D1_AW_AC_NRT_S1 << 16) | (D1_AW_AC_RT_S0 << 8) | (D1_AW_IC_M0)) ;
	__raw_writel(val_r, base + XIU1_AW_TIDEMARK);
...
static void bts_drex_init(void __iomem *base)
{
	...
	__raw_writel(0x00C80000, base + QOS_TIMEOUT_0xF);
	__raw_writel(0x00C80010, base + QOS_TIMEOUT_0xD);
	__raw_writel(0x00C80010, base + QOS_TIMEOUT_0xC);
	__raw_writel(0x00C80020, base + QOS_TIMEOUT_0xB);
	__raw_writel(0x00C80040, base + QOS_TIMEOUT_0xA);
	__raw_writel(0x00C80060, base + QOS_TIMEOUT_0x8);
	__raw_writel(0x00C80200, base + QOS_TIMEOUT_0x6);
	__raw_writel(0x00C80FFF, base + QOS_TIMEOUT_0x4);
	__raw_writel(0x00C80FFF, base + QOS_TIMEOUT_0x0);

	__raw_writel(0x00000011, base + BRB_RSVCONTROL);
	__raw_writel(0x08080808, base + RBRB_RSVCONFIG);
	__raw_writel(0x08080802, base + WBRB_RSVCONFIG);
	__raw_writel(0x00000000, base + DATA_RSVCONTROL);

/* Unprogrammed QoS values */
#if 0
	__raw_writel(0x00000004, base + QOS_TIMEOUT_0xE);
	__raw_writel(0x00000020, base + QOS_TIMEOUT_0x9);
	__raw_writel(0x00000020, base + QOS_TIMEOUT_0x7);
	__raw_writel(0x00000020, base + QOS_TIMEOUT_0x5);
	...
	__raw_writel(0x00006060, base + RDATA_RSVCONFIG);
	__raw_writel(0x00008060, base + WDATA_RSVCONFIG);
#endif
}
		...
		void *ptr)
{
	...
	case PM_POST_SUSPEND:
		...
		bts_xiu_init(base_sysreg_ccore);
...
{
	base_drex0 = ioremap(EXYNOS7880_PA_DREX0, SZ_4K);
	base_drex1 = ioremap(EXYNOS7880_PA_DREX1, SZ_4K);
	base_bts_pmu_alive = ioremap(EXYNOS7880_PA_PMU_ALIVE, SZ_4K);
	base_sysreg_ccore = ioremap(EXYNOS7880_PA_SYSREG_CCORE, SZ_4K);
	base_vpp_mo = ioremap(EXYNOS7880_PA_VPP_MO, SZ_32K);
	base_isp_xiu = ioremap(EXYNOS7880_PA_SYSREG_ISP, SZ_8K);
	base_mfcscl_sysreg = ioremap(EXYNOS7870_PA_SYSREG_MFCMSCL, SZ_8K);
}
		...
		unsigned int bw, int bw_type)
{
	...
	case TYPE_CAM:
		...
		break;
	default:
		pr_err("BTS(MIF) : unsupported media_type - %u", media_type);
		break;
	}

	total_bw = decon_bw + cam_bw;

	/* MIF and INT minimum frequency derivation */

	if (cam_bw) {
		if (total_bw > (1400000000)) {
			mif_freq = 1539000;
			int_freq = 400000;
		} else if (total_bw > 663800000) {
			mif_freq = 1144000;
			int_freq = 400000;
		} else {
			mif_freq = 546000;
		...
		if (total_bw > 1705000000) {
			mif_freq = 1539000;
			int_freq = 267000;
		} else if (total_bw > 1455000000) {
			mif_freq = 1352000;
			int_freq = 134000;
		} else if (total_bw > 1050000000) {
			mif_freq = 1014000;
			int_freq = 134000;
		} else if (total_bw > 850000000) {
			mif_freq = 845000;
			int_freq = 134000;
		} else if (total_bw > 663800000) {
			mif_freq = 676000;
...
void bts_update_winlayer(unsigned int layers)
{
	...
	if (pm_qos_request_active(&exynos7_winlayer_mif_bts_qos))
		pm_qos_update_request(&exynos7_winlayer_mif_bts_qos, freq);
...
static int __init exynos7_bts_init(void)
{
	...
	for (i = BS_DEFAULT + 1; i < BS_MAX; i++) {
		scen_chaining(i);
		BTS_DBG("[BTS][%s] scene(%d) is chained\n", __func__, i);
	}
	...
	pm_qos_add_request(&exynos7_winlayer_mif_bts_qos, PM_QOS_BUS_THROUGHPUT, 0);
