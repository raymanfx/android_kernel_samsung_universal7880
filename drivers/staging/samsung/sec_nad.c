        ...
        char *buf)
{
    ...
    if (!strncasecmp(sec_nad_env.nad_result, "NG", 2))
        ...
        const char *buf, size_t count)
{
    ...
    if (!strncmp(buf, "erase", 5)) {
        strcpy(sec_nad_env.nad_factory, "GAP");
        ...
        char *buf)
{
    ...
    if (!strncasecmp(sec_nad_env.nad_acat_result, "NG", 2))
        ...
        const char *buf, size_t count)
{
    ...
    if (!strncmp(buf, "nad_acat", 8)) {
        ...
        if (!strncmp(nad_cmd[2], "1", 1))
            strcpy(sec_nad_env.nad_dram_test_need, "DRAM");
        else
            strcpy(sec_nad_env.nad_dram_test_need, "DONE");
